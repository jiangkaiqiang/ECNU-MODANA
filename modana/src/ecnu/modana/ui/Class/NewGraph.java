...
public class NewGraph extends MuiGArea {
	...
	public Node createNode(double x, double y,int index) {
		...
		// draw text
		text = new Text(x + textInsets.getLeft(), y + textInsets.getTop(),
				"Class1");
		...
		polyline.setTranslateX(rect.getX());
		polyline.setTranslateY(rect.getY());
		text2 = new Text("ah");
		text2.setX(rect.getX() + 10);
		text2.setY(rect.getY() + 80);

		Group g = new Group(rect, text, polyline);
	...
	public void updateNode(double diffX, double diffY, MOUSE_ACTION action) {
		...
		case S_RESIZE:
			...
			break;
		case RELOCATE:
			graphicNode.setTranslateX(diffX);
			graphicNode.setTranslateY(diffX);
			break;
		default:
			return;
		}
