...
public class PrismState extends MuiGArea {
	//saving created node
...
//	return propertyWindow;
		if (propertyWindow == null) 
		{		
			PropWindow ppw = new PropWindow("name:","index:");
			propertyWindow = ppw.getPropertyWindow();
			//ppw.getTextField().setText(text.getText());
			ppw.getOkButton().setOnAction(new EventHandler<ActionEvent>() {		
				@Override
				public void handle(ActionEvent e) {
					//change data
					text.setText(name.getValue()+index.getValue());
					propertyWindow.close();
				}
			});
			int i=0;
			ppw.tfList.get(i++).textProperty().bindBidirectional(name);
			ppw.tfList.get(i++).textProperty().bindBidirectional(index);
		}
		return propertyWindow;
	}
	...
	public Node createNode(double x, double y,int index) {
		//name.setValue("s");
		this.index.setValue(String.valueOf(index));
		...
		//draw text
		text = new Text(x+textInsets.getLeft(), y+textInsets.getTop(), name.getValue()+index);
